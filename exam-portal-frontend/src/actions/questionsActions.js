...
// Student exam questions (answers stripped, subset/randomized on the backend).
export const fetchExamQuestions = async (dispatch, quizId, token) => {
  dispatch({ type: questionsConstants.FETCH_QUESTIONS_REQUEST });
  const data = await questionsServices.fetchExamQuestions(quizId, token);
  if (data) {
    return dispatch({
      type: questionsConstants.FETCH_QUESTIONS_SUCCESS,
      payload: data,
