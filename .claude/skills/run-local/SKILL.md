...
- **MySQL on `:3306`**. The DB `exam-portal` is auto-created (`createDatabaseIfNotExist=true`),
  so the server only needs to be up with credentials that match — default `root` / `root@`.
...
```
On Windows PowerShell use `$env:JWT_SECRET="..."` etc. Flyway applies `V1..V7` on startup;
watch the log for `Successfully applied` and `Started ExamPortalBackendApplication`.
...
Open `http://localhost:3000`, register a school/admin or log in as the seeded super admin,
create a category → quiz → question, then confirm it appears. A blank page = check the browser
console and the backend log. If `/api` calls 404 in the browser but `curl` to `:8081` works,
the CRA proxy (`src/setupProxy.js`) isn't running — you're hitting the built bundle, not `npm start`.
