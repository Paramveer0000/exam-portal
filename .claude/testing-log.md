...
### 2026-08-11 18:05 — Message to Parents page + R.I.A.S.E.C analysis section  [type: change]
- what: client listed two gaps against the reference report: (1) a "Message to Parents" letter on page 3, (2) a dedicated RIASEC analysis section. Both added; template-only, no Java/DTO/DB change.
- Message to Parents: new section between "Your Report at a Glance" and "How to Read", so it lands on page 3. Copy is the client's verbatim letter and is IDENTICAL in every report, so it lives in report.html, NOT in content/report-content.json (that file holds per-report variable copy). Only the sign-off tracks brand.name. Includes the client's "not for medical purpose" line as the disclaimer.
- R.I.A.S.E.C analysis: new section rendered from the EXISTING RIASEC dimension group (dimensionGroups.?[type == 'RIASEC']), so it recomputes nothing -- same percentages the group section already printed, re-presented in theory order R I A S E C. Contains a two-column score summary, a DOMAIN CODE block from group.top (leading three), and six full-colour letter panels (colour fixed per Holland letter) each carrying Personality / Values / Potential aptitudes / Learning style / Environment.
- the six-type theory copy is a SpEL map literal in the template keyed by dimensionCode (R/I/A/S/E/C from V26). It is fixed theory, not per-student content -- do not move it to the AI content path.
- section disappears entirely when a report carries no RIASEC dimensions (guarded on the selection being non-empty), so pre-RIASEC assessments are unaffected.
- FIXTURE: ReportRenderHarnessTest now supplies a RIASEC group as well as the MI group, otherwise the new section would never be exercised in QA. Numbers are literals, as with every other fixture value.
...
### 2026-08-27 14:56 — Premium reports restricted; impersonation return survives refresh  [type: change]
- what: AI-summary generation and PDF create/rebuild are SUPER_ADMIN-only; schools/students can only download an issued PDF and see "Report not prepared yet." otherwise. Fixed intermittent Return to Super Admin failure after the 30-minute access-token refresh by persisting signed impersonator identity through refresh-token rotation; failed returns stay retryable, successful returns revoke the impersonated refresh token, and logout/session-expiry clears stale recovery state.
- files: SecurityConfig, PsychometricReportServiceImpl, MentalistReportServiceImpl, PsychometricReportPage, Header, AuthServiceImpl, AdminServiceImpl/controller/interface, RefreshToken/RefreshTokenService, V31__refresh_token_impersonator.sql, AI settings copy, auth/api cleanup, focused backend/frontend tests.
- result: TDD red/green confirmed. Backend non-DB suite 91/91 green (`-Dtest=!ExamPortalBackendApplicationTests`); focused React suites 4/4 green; normal `npm run build` exit 0 (repository-wide pre-existing ESLint warnings remain); `git diff --check` clean apart from line-ending notices.
- notes / follow-ups: V31 was compiled/validated but not applied to the live populated MySQL database in this session; restart backend and confirm Flyway applies V31 before testing impersonation in the browser. `contextLoads` intentionally excluded as in prior sessions.
